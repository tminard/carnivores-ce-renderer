...
TBEGIN:
  double targetx = this->tgx;
  double targetz = this->tgz;
  double targetdx = targetx - this->character->getCurrentPosition().x;
  double targetdz = targetz - this->character->getCurrentPosition().z;
  
  double tdist = sqrt( targetdx * targetdx + targetdz * targetdz );
...
ENDPSELECT:
  ...
  double rspd, currspeed, tgbend, new_alpha;
  double dalpha = (float)fabs(this->TargetAlpha - this->character->getCurrentAlpha());
  double drspd = dalpha;
  ...
  if (this->AfriadTime) DeltaFunc(this->rspeed, currspeed, time_delta / 160.f);
  else DeltaFunc(this->rspeed, currspeed, time_delta / 180.f);
  
  tgbend = drspd/2;
  if (tgbend>M_PI/5) tgbend = M_PI/5;
  
  tgbend*= SGN(currspeed);
  if (fabs(tgbend) > fabs(this->bend)) DeltaFunc(this->bend, tgbend, time_delta / 800.f);
  else DeltaFunc(this->bend, tgbend, time_delta / 600.f);
